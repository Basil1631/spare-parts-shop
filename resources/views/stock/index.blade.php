<x-layouts.app title="Stock">
    <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
        <div>
            <p class="text-sm text-slate-500">On-hand quantities for this shop. Incoming stock is confirmed on Incoming stock. Damage is adjusted out here by godown staff.</p>
        </div>
        <div class="flex gap-2">
            ...
            @if (auth()->user()->canPurchase())
                <a href="{{ route('purchases.create') }}" class="px-3 py-2 rounded-xl text-sm bg-teal-600 text-white">Raise purchase</a>
            @endif
        </div>
    </div>
    ...
    @if ($products->isEmpty())
        <p class="text-slate-600 bg-white rounded-2xl p-6">No matching products. Ask admin to add the SKU first.</p>
    @endif
    <div class="space-y-3">
        @foreach ($products as $product)
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 {{ $product->isLowStock() ? 'ring-2 ring-red-400' : '' }}">
                <div class="flex flex-wrap justify-between gap-4">
                    ...
                    @if (auth()->user()->canManageGodownStock())
